...
def comparePressureEigenvectorAlignment(gt_ph, arr_pred_ph, labels=[], title=''):
    ...
    # Secret sauce: nbins=30 over the three eigen-pairs (e1/e2/e3); the
    # alignment angle theta = arccos|<e_pred, e_gt>| lives on [0, pi/2].
    nbins = 30;
    for j in range(num_comps):
        eig_align = calcEigenvectorAlignment(gt_ph, arr_pred_ph[i]);
...
def plotPressureEigenvectorAlignment2d(ax, gt, pred, eig1=0, eig2=3, density=True):
...
def plotDistribution(ax, arr,
                     ...
                     density=True,
                     label='model'):
    """PDF of a single VGT component across samples, rms-normalized.

    arr - 1D tensor of one component over the sample dimension, e.g.
          ``vgt[:, 0, 0]`` (A11) or ``vgt[:, 0, 1]`` (A12); shape (N,).

    Secret sauce (tuned in visualize.ipynb, preserved here):
    - normalize by the rms ``arr / sqrt(<arr^2>)`` (NOT 1/sqrt(sum)): this
      sets unit variance so DNS and model curves are compared by *shape*, not
      magnitude. Use mean, not sum, so the scaling is sample-count invariant.
    - bins=100, xrange=(-15.5, 15.5): a fixed window shared across all curves
      so the heavy non-Gaussian tails line up for overlay comparison.
    - log=True: the VGT-component PDFs span several decades (intermittent tails).
    - histtype='step': lets DNS/TBNN/LDM overlay cleanly on one axis.
    The caller typically follows with ``ax.set_xlim(-15, 15)`` and ``ax.legend()``.
    """
    def normalize(_arr):
        return torch.div(_arr, torch.sqrt(torch.mean(_arr**2)))
    ax.hist(normalize(arr).cpu().numpy(), bins=bins, density=density, log=log,
            range=xrange, histtype=histtype, label=label)
    return

def plotLongitudinalPDF(ax, vgt, label='model', **kwargs):
    """Longitudinal VGT-component PDF: A11 = du_1/dx_1 = vgt[:, 0, 0]."""
    return plotDistribution(ax, vgt[:, 0, 0], label=label, **kwargs)

def plotTransversePDF(ax, vgt, label='model', **kwargs):
    """Transverse VGT-component PDF: A12 = du_1/dx_2 = vgt[:, 0, 1]."""
    return plotDistribution(ax, vgt[:, 0, 1], label=label, **kwargs)
...
def plot_qr_pdf(ax, vgt, num_bins=100, levels=7, linestyle='solid'):
    # Secret sauce (tuned in visualize.ipynb, preserved here):
    # - vgt must be permuted to (3, 3, N) for the trailing-axis einsums below.
    # - rlims/qlims are the *normalized* (little q-r) teardrop bounds, fixed by
    #   the unit-norm constraint b = A/||A||_F: r in (-sqrt(3)/9, sqrt(3)/9),
    #   q in (-1/2, 1/2). Do not widen these — they are the physical extent.
    # - good contour levels for the a-posteriori VGT PDF are geometric, NOT the
    #   default 7: pass levels=[2**i for i in range(-10, 10)] with num_bins=35.
    colormap = plt.rcParams['axes.prop_cycle'].by_key()['color'];
    rlims = (-np.sqrt(3)/9, np.sqrt(3)/9);
    qlims = (-1/2, 1/2);
    r_range = np.linspace(rlims[0], rlims[1], num=num_bins);
