...
class LATN_NODE(nn.Module):
    ...
    def forward_multistep(self, aij_cont_series, timescale, num_steps):
        ...
        NOTE [flagged port]: ported verbatim from the HPC source. Its body still
        calls ``self.forward(x, timescale)`` with the pre-port 2-arg signature,
        but ``forward`` now takes a single ``inputs`` argument. This method is
        superseded by ``forward``/``forward_eval`` and is not on the training
        path; left unrepaired pending review (see flag list).
        """
        history_sample_inds = lagrdataset._create_inds(
            self.data_desc.history_length,
            self.data_desc.history_timestep,
