...
def multiprocess_train(rank: int,
    ...
    trainer = training_utils.Trainer(
        model, train_dl, test_dl, train_desc.optimizer, train_desc.learning_rate,
        train_desc.scheduler, rank, train_desc.save_every, train_desc.savepath)
...
@ut.skip("Trainer exercises the DDP train/test loop (all_gather, world_size) "
         "and needs a live nccl process group + GPU; not runnable under the "
         "CPU unittest path. Also stale vs the current Trainer signature.")
